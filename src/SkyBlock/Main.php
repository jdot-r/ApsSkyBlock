...
class Main extends Base implements Listener{
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args){
		if(strtolower($command->getName()) == "is"){
			if(!(isset($args[0]))){
				$sender->sendMessage(TextFormat::YELLOW . "please use subcommand");
				...
				if($args[0] == "help"){
					if($sender->hasPermission("is") || $sender->hasPermission("is.help")){
						if(!(isset($args[1])) or $args[1] == "1"){
							...
							$sender->sendMessage(TextFormat::YELLOW . "/is find (For-Admins)");
							return true;
						}elseif($args[1] == "2"){
					...
					if($sender->hasPermission("is") || $sender->hasPermission("is.command") || $sender->hasPermission("is.command.remove")){
						if(!(isset($args[1]))){
							$sender->sendMessage("Are you Really Want remove Your Islands? Use /is remove yes to confirm");
							return true;
						}elseif($args[1] == "yes"){
								if(file_exists($this->getDataFolder()."Islands/".$sender->getName().".txt")){
									unlink($this->getDataFolder()."Islands/".$sender->getName().".txt");
									$sender->sendMessage("Your island has been removed");
									return true;
								}else{
								...
								$sender->sendMessage("Island Removing Canceled!");
								return true;
							}else{
								return false;
							}
						}else{
							$sender->sendMessage("You cant remove your island");
							return true;
						}
	...
	public function makeIsland($name){
		...
		if(!($player instanceof Player)){
			...
			// 1st side
			$levelName->setBlock(new Vector3($randX, $Y-2, $randZ), new Stone());
			$levelName->setBlock(new Vector3($randX+6, $Y-2, $randZ+6), new Stone());
			$levelName->setBlock(new Vector3($randX+6, $Y-2, $randZ+5), new Stone());
			$levelName->setBlock(new Vector3($randX+6, $Y-2, $randZ+4), new Stone());
			$levelName->setBlock(new Vector3($randX+6, $Y-2, $randZ+3), new Stone());
			$levelName->setBlock(new Vector3($randX+6, $Y-2, $randZ+2), new Stone());
			$levelName->setBlock(new Vector3($randX+6, $Y-2, $randZ+1), new Stone());
			
			// 2nd side
			$levelName->setBlock(new Vector3($randX+5, $Y-2, $randZ+6), new Stone());
			$levelName->setBlock(new Vector3($randX+5, $Y-2, $randZ+5), new Stone());
			$levelName->setBlock(new Vector3($randX+5, $Y-2, $randZ+4), new Stone());
			$levelName->setBlock(new Vector3($randX+5, $Y-2, $randZ+3), new Stone());
			$levelName->setBlock(new Vector3($randX+5, $Y-2, $randZ+2), new Stone());
			$levelName->setBlock(new Vector3($randX+5, $Y-2, $randZ+1), new Dirt());
			
			// 3rd side
			$levelName->setBlock(new Vector3($randX+4, $Y-2, $randZ+6), new Stone());
			$levelName->setBlock(new Vector3($randX+4, $Y-2, $randZ+5), new Stone());
			$levelName->setBlock(new Vector3($randX+4, $Y-2, $randZ+4), new Stone());
			$levelName->setBlock(new Vector3($randX+4, $Y-2, $randZ+3), new Stone());
			$levelName->setBlock(new Vector3($randX+4, $Y-2, $randZ+2), new Stone());
			$levelName->setBlock(new Vector3($randX+4, $Y-2, $randZ+1), new Stone());
			
			// 4th side
			$levelName->setBlock(new Vector3($randX+3, $Y-2, $randZ+6), new Stone());
			$levelName->setBlock(new Vector3($randX+3, $Y-2, $randZ+5), new Stone());
			$levelName->setBlock(new Vector3($randX+3, $Y-2, $randZ+4), new Stone());
			$levelName->setBlock(new Vector3($randX+3, $Y-2, $randZ+3), new Stone());
			$levelName->setBlock(new Vector3($randX+3, $Y-2, $randZ+2), new Stone());
			$levelName->setBlock(new Vector3($randX+3, $Y-2, $randZ+1), new Stone());
			
			// 5th side
			$levelName->setBlock(new Vector3($randX+2, $Y-2, $randZ+6), new Stone());
			$levelName->setBlock(new Vector3($randX+2, $Y-2, $randZ+5), new Stone());
			$levelName->setBlock(new Vector3($randX+2, $Y-2, $randZ+4), new Stone());
			$levelName->setBlock(new Vector3($randX+2, $Y-2, $randZ+3), new Stone());
			$levelName->setBlock(new Vector3($randX+2, $Y-2, $randZ+2), new Stone());
			$levelName->setBlock(new Vector3($randX+2, $Y-2, $randZ+1), new Stone());
			
			// 6th side
			$levelName->setBlock(new Vector3($randX+1, $Y-2, $randZ+6), new Stone());
			$levelName->setBlock(new Vector3($randX+1, $Y-2, $randZ+5), new Stone());
			$levelName->setBlock(new Vector3($randX+1, $Y-2, $randZ+4), new Stone());
			$levelName->setBlock(new Vector3($randX+1, $Y-2, $randZ+3), new Stone());
			$levelName->setBlock(new Vector3($randX+1, $Y-2, $randZ+2), new Stone());
			$levelName->setBlock(new Vector3($randX+1, $Y-2, $randZ+1), new Stone());
			...
			$player->sendMessage(TextFormat::GREEN . "Use /is remove");
			$player->sendMessage(TextFormat::GREEN . "Then create a new island");
